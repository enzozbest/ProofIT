...
describe('OldPrompts', () => {
  ...
  it('truncates long text with line-clamp-3', () => {
    ...
    const longTextElement = screen.getByText(
      'Creating A Web Page From Scratch That Is Fully Responsive and Optimized for SEO'
    );
  ...
  it('renders within a parent container', () => {
    ...
    expect(parentContainer).toContainElement(
      screen.getByText('Generating Code For An Application')
    );
  });
});
