...
class OllamaServiceTest {
    ...
    @Test
    fun `test generateResponse handles invalid JSON`() = testApplication {
        val invalidJsonEngine = MockEngine {
            respond(
                content = """{
                    "model": "codellama:7b",
        ...
        val invalidService = OllamaService().apply {
            client = HttpClient(invalidJsonEngine) {
    ...
    @Test
    fun `test generateResponse when Ollama is not running`() = testApplication {
        val unhealthyEngine = MockEngine {
            respond("Not Found", HttpStatusCode.NotFound)
        }

        val unhealthyService = OllamaService().apply {
    ...
    @Test
    fun `test generateResponse handles HTTP errors`() = testApplication {
        ...
        val errorService = OllamaService().apply {
    ...
    @Test
    fun `test successful response parsing`() = testApplication {
        ...
        val successService = OllamaService().apply {
            client = HttpClient(successEngine) {
                install(ContentNegotiation) {
                    json(Json {
                        prettyPrint = true
                        isLenient = true
        ...
        assertEquals("<html>Test</html>", result.getOrNull()?.files?.get("index.html")?.content)
    }

    @Test
    fun `test generateResponse handles network exception during API call`() = testApplication {
        val throwingEngine = MockEngine { request ->
            when (request.url.encodedPath) {
        ...
        val throwingService = OllamaService().apply {
            client = HttpClient(throwingEngine) {
                install(ContentNegotiation) {
                    json(Json { ignoreUnknownKeys = true })
                }
            }
