...
object TemplateRetrieval {
     ...
     */
    suspend fun storeTemplateFiles(
     ...
     */
    fun generateTemplateAnnotation(
        templateCode: String,
        componentName: String,
    ): String? =
        runBlocking {
            try {
                ...
                val jsonText = (response as OllamaResponse).response ?: throw PromptException("Empty response from LLM")
                val jsonElement = Json.parseToJsonElement(jsonText)

                when (jsonElement) {
                    is JsonObject -> {
                        val annotation =
                            jsonElement["annotation"]?.jsonPrimitive?.content
                    ...
                    is JsonArray -> {
                        ...
                        val annotation =
                            firstObject["annotation"]?.jsonPrimitive?.content
                                ?: throw PromptException("No annotation field in LLM response")
                        return@runBlocking annotation
                    }
